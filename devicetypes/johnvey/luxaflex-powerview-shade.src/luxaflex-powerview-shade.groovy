...
metadata {
    definition (
        name: "Luxaflex PowerView Shade", 
        namespace: "johnvey", 
        author: "Johnvey Hwang & Chris Howell",
        vid: "generic-shade",
    ...
    tiles(scale: 2) {
        ...
        multiAttributeTile(name:"windowShade", type: "blind", width: 6, height: 4, canChangeIcon: true) {
            tileAttribute ("device.windowShade", key: "PRIMARY_CONTROL") {
                attributeState "open", label:'Open', action:"windowShade.close", icon:"st.shades.shade-open", backgroundColor:"#00a0dc", nextState:"closing"
                attributeState "partially open", label:'Partially Open', action:"windowShade.close", icon:"st.shades.shade-open", backgroundColor:"#00a0dc", nextState:"closing"
                attributeState "closed", label:'', action:"windowShade.open", icon:"st.shades.shade-close", backgroundColor:"#ffffff", nextState:"opening"
                attributeState "opening", label:'Opening', action:"windowShade.close", icon:"st.shades.shade-opening", backgroundColor:"#00a0dc", nextState:"partially open"
                attributeState "closing", label:'Closing', action:"windowShade.open", icon:"st.shades.shade-closing", backgroundColor:"#ffffff", nextState:"partially open"
            }
        ...
        // jog shade
        standardTile("jog", "device.windowShade", width: 2, height: 2, inactiveLabel: false, decoration: "flat") {
            state("default", label:'Jog shade', action:"jog", icon:"st.motion.motion.inactive")
        }
...
def sendRequestCallback(response) {
    ...
    log.debug "json response: $response.json"
...
def parseShadeData(payload) {
    ...
    if (shade.positions.posKind1 == ShadeComponentType.SHADE) {
        def shadeLevel = (shade.positions.position1 / ShadeMaxPosition.SHADE * 100) as Integer
        def shadeValue = null       
        if (shadeLevel >= 99) {
        	shadeLevel = 100
        	shadeValue = "open"
    	...
    	def levelEvent = sendEvent(name: "level", value: shadeLevel, unit: "%")
    	def stateEvent = sendEvent(name: "windowShade", value: shadeValue, descriptionText: descriptionText, isStateChange: true)
    }
...
/**
 * Sets the blinds to the preset position
 */
def presetPosition() {
    log.debug "Executing 'presetPosition'"
    return setPosition(preset ?: state.preset ?: 20, ShadeComponentType.SHADE)
}
